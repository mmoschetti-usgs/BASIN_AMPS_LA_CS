...
% BA-amplification file
ampFile=sprintf('ampBA_varVs30_760_%ds.csv',period);
fid2=fopen(ampFile,'w');
fprintf(fid2,'lon,lat,Vs30-Wills,Z2.5,Z1,ln-Amp760\n');
...
%    fprintf(fid2,'%.4f,%.4f,%s,%.1f,%.1f,%.5f\n',lon(cnt),lat(cnt),Vs30_wills2015(cnt),Z2p5(cnt),Z1(cnt),-ln_amp);
  else
    fprintf(fid,'%.4f,%.4f,%.5f,%.1f,%.1f,%.1f,%.5f\n',lon(cnt),lat(cnt),Bvalue(cnt),Vs30_wills2015(cnt),Z2p5(cnt),Z1(cnt),modBvalue);
    fprintf(fid2,'%.4f,%.4f,%.1f,%.1f,%.1f,%.5f\n',lon(cnt),lat(cnt),Vs30_wills2015(cnt),Z2p5(cnt),Z1(cnt),-ln_amp);
  end
