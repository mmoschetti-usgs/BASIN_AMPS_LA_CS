function [gm_lin]=calc_BSSA_Z1_sens(vs30,Z2p5,Z2p5mod,period)
...
input.z2p5  =   Z2p5; % in km; NaN triggers default basin depth model
...
input.vs30  = vs30; % in m/s
input.vsInf =  true; % boolean
input.z2p5  =   Z2p5mod; % in km; NaN triggers default basin depth model
%input.z1p0  =   Z1mod; % in km; NaN triggers default basin depth model
% Do a calculation. The MatUtil.calc(gmm, imt, gmmInput) method returns an
% array of [ln(median ground motion), sigma]
result = hazMat.gmmMean(gmm, imt, input);
gm_z1mod = result(1);
gm_log=gm_z1mod-gm_z1;
gm_lin=exp(gm_log);
